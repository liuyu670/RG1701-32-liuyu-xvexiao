...
layui.config({
    ...
    //表格数据
    table.render({
        id: 'tcourseList',//设定容器唯一 id。id 是对表格的数据操作方法上是必要的传递条件，它是表格容器的索引，
        elem: '#tcourseList',//指定原始 table 容器的选择器或 DOM，方法渲染方式必填
        url:ctx+'/teacher/getCourseList', //数据接口 默认会自动传递两个参数：?page=1&limit=30 page 代表当前页码、limit 代//表
    ...
    table.on('tool(test)',function (obj) { //test 是 table 原始容器的属性 lay-filter="对应的值
            ...
            layer.confirm('真的删除吗？',function (index) {
                $.ajax({
                    ...
                    success:function (d) {
                        if(d.code==0) //删除成功
                        {
                            table.reload('tcourseList',{}) //表格重载 这里的表格重载是指对表格重新进行渲染
        ...
        else if(obj.event==='edit')
        {
            layer.open({ //发送请求 controller返回一个jsp页面 加载在这个open的弹出窗口中
                ...
                title:"编辑课程",
                area:['380px','460px'],
                content:ctx+"/teacher/editCourse/"+data.id  // 发送的是/admin/editAdmin/4
            })
        }
    });
    //添加角色 通过选择样式 选择
    ...
    $(".courseAdd_btn").click(function(){
        var index = top.layer.open({     //这里不能用layui.layer.open 否则不是在最上层 会被挡住一部分
            title : "添加课程",
            type : 2,
            content : ctx+"/teacher/addTCourse",
            area: ['450px', '570px'],
